...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define FLASH_TEST_ADDRESS     0x000000UL
#define FLASH_TEST_ERASE_SIZE  4096UL
...
/* USER CODE BEGIN PV */
static const uint8_t flash_test_pattern[] =
    "W25Q64 SPI NOR self test from Example/test_spi";
static uint8_t flash_test_readback[ sizeof( flash_test_pattern ) ];

volatile StorageStatus g_flash_init_status = STORAGE_NOT_INITIALIZED;
volatile StorageStatus g_flash_info_status = STORAGE_NOT_INITIALIZED;
volatile StorageStatus g_flash_erase_status = STORAGE_NOT_INITIALIZED;
volatile StorageStatus g_flash_program_status = STORAGE_NOT_INITIALIZED;
volatile StorageStatus g_flash_read_status = STORAGE_NOT_INITIALIZED;
volatile uint32_t g_flash_compare_ok = 0U;
volatile uint64_t g_flash_capacity_bytes = 0U;
volatile uint32_t g_flash_page_size_bytes = 0U;
volatile uint32_t g_flash_erase_size_bytes = 0U;
...
/* USER CODE BEGIN PFP */
static int run_flash_self_test( void );
...
static int run_flash_self_test( void )
{
  ...
  g_flash_init_status = STORAGE_NOT_INITIALIZED;
  g_flash_info_status = STORAGE_NOT_INITIALIZED;
  g_flash_erase_status = STORAGE_NOT_INITIALIZED;
  g_flash_program_status = STORAGE_NOT_INITIALIZED;
  g_flash_read_status = STORAGE_NOT_INITIALIZED;
  g_flash_compare_ok = 0U;
  g_flash_capacity_bytes = 0U;
  g_flash_page_size_bytes = 0U;
  g_flash_erase_size_bytes = 0U;

  memset( flash_test_readback, 0, sizeof( flash_test_readback ) );

  g_flash_init_status = board_flash_init();
  if( g_flash_init_status != STORAGE_OK )
  {
    return 0;
  }

  g_flash_info_status = storage_get_info( g_board_flash, &info );
  if( g_flash_info_status != STORAGE_OK )
  {
    return 0;
  }

  g_flash_capacity_bytes = info.capacity_bytes;
  g_flash_page_size_bytes = info.page_size_bytes;
  g_flash_erase_size_bytes = info.erase_size_bytes;

  g_flash_erase_status = storage_erase( g_board_flash,
                                        FLASH_TEST_ADDRESS,
                                        FLASH_TEST_ERASE_SIZE );
  if( g_flash_erase_status != STORAGE_OK )
  {
    return 0;
  }

  g_flash_program_status = storage_program( g_board_flash,
                                            FLASH_TEST_ADDRESS,
                                            flash_test_pattern,
                                            sizeof( flash_test_pattern ) );
  if( g_flash_program_status != STORAGE_OK )
  {
    return 0;
  }

  g_flash_read_status = storage_read( g_board_flash,
                                      FLASH_TEST_ADDRESS,
                                      flash_test_readback,
                                      sizeof( flash_test_readback ) );
  if( g_flash_read_status != STORAGE_OK )
  {
    return 0;
  }

  g_flash_compare_ok =
      ( memcmp( flash_test_pattern,
                flash_test_readback,
                sizeof( flash_test_pattern ) ) == 0 ) ? 1U : 0U;

  return ( g_flash_compare_ok != 0U ) ? 1 : 0;
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  board_init();
  flash_test_ok = run_flash_self_test();
