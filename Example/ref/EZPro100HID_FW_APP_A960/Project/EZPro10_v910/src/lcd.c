...
void displayfuntion()
{
	...
	for (i=0;i<10;i++)
	{
		if ((MCU_Config_xx.ModeIn_CONFIG_xx.MCU_Name[i]>0x29) && (MCU_Config_xx.ModeIn_CONFIG_xx.MCU_Name[i]<0x7b))
		{
			b=8*i;
			a=41+b;
			displayAZ(pp[MCU_Config_xx.ModeIn_CONFIG_xx.MCU_Name[i]-0x30],0x00,0x01,a); //MCXXXXXX
		}
	}
	//CheckSum Option
    ...
    if (iic_data[0] !=0xff)	//0xa0 ,0xaa,0x0a      
	{
...
void display_CRC()
{
	...
	b=b%0x10;
	displayAZ(pNum[a],0X02,0X03,41); 
	displayAZ(pNum[b],0X02,0X03,49); 

	displayAZ(lcd_blank,0X02,0X03,57);

	//otption CRC
	displayAZ(lcd_O,0X02,0X03,65);
	displayAZ(lcd_P,0X02,0X03,73);
	displayAZ(lcd_T,0X02,0X03,81);  
	displayAZ(lcd_maohao,0x02,0x03,89);
	a=OptionCS/0x1000;
	b=OptionCS%0x1000;

	displayAZ(pNum[a],0X02,0X03,97);
	a=b/0x100;
	b=b%0x100;
	displayAZ(pNum[a],0X02,0X03,105);
	a=b/0x10;
	b=b%0x10;
	displayAZ(pNum[a],0X02,0X03,113); 
	displayAZ(pNum[b],0X02,0X03,121); 
...
void displayErorr(u8 erorrNum)
{
	...
	displayCHAR(lcd_cuo,6,7,1);
	displayCHAR(lcd_wu,6,7,17);
	displayCHAR(lcd_dai,6,7,33);
	displayCHAR(lcd_ma,6,7,49);
	displayAZ(lcd_maohao,6,7,65);

	displayAZ(lcd_blank,6,7,73);
	displayAZ(lcd_blank,6,7,81);

	displayAZ(pp[b],6,7,89);
	displayAZ(pp[s],6,7,97);
	displayAZ(pp[g],6,7,105);

	displayAZ(lcd_blank,6,7,113);
	displayAZ(lcd_blank,6,7,121);
